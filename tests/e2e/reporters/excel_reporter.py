...
def generate_excel_report(test_results, output_path):
    """Generates the primary Automation_Test_Report.xlsx containing 6 worksheets."""
    ...
    apply_header_style(ws1, "Executed Web E2E Test Cases - Selenium WebDriver", 8)
    headers = ["Test ID", "Module", "Test Name", "Status", "Execution Time (s)", "Priority", "Timestamp", "Errors/Console Logs"]
    ...
    apply_header_style(ws2, "Passed Web E2E Test Cases Record", 7)
    ...
    apply_header_style(ws3, "Failed Web E2E Test Cases Record", 8)
    ...
    apply_header_style(ws4, "Skipped Web E2E Test Cases Record", 8)
    ...
    apply_header_style(ws5, "High-Level Web Automation Metrics Summary", 2)
    ...
    metrics_data = [
        ...
        ("Browser Under Test", "Headless Google Chrome"),
        ("Live Deployment URL", os.getenv("BASE_URL", "https://durga1610.github.io/kisan-mitra-web/")),
    ...
    apply_header_style(ws6, "Failed Web Tests & Error Log Summary", 5)
    ws6.row_dimensions[3].height = 24
    def_headers = ["Defect ID", "Failed Test ID", "Module", "Failure Reason", "Priority"]
    ...
    for r_idx, t in enumerate(failed, 4):
        ...
        ws6.cell(row=r_idx, column=5, value=t["priority"]).border = thin_border
    autofit_columns(ws6)
    
    wb.save(output_path)
    print(f"[ExcelReporter] Primary report saved successfully to {output_path}")
...
def generate_passed_report(test_results, output_path):
    ...
    apply_header_style(ws, "Passed Web E2E Test Cases Record", 7)
    headers = ["Test ID", "Module", "Test Name", "Status", "Execution Time (s)", "Priority", "Timestamp"]
    ...
    autofit_columns(ws)
    wb.save(output_path)
    print(f"[ExcelReporter] Passed report saved to {output_path}")
...
def generate_failed_report(test_results, output_path):
    ...
    apply_header_style(ws, "Failed Web E2E Test Cases Record", 8)
    ...
    autofit_columns(ws)
    wb.save(output_path)
    print(f"[ExcelReporter] Failed report saved to {output_path}")
...
def generate_summary_report(test_results, output_path):
    ...
    apply_header_style(ws, "Web E2E Execution Metrics Summary", 5)
    ...
    autofit_columns(ws)
    wb.save(output_path)
    print(f"[ExcelReporter] Summary report saved to {output_path}")
