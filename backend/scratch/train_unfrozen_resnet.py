...
# Save to plant_disease_resnet_new.pt to avoid overwriting production prematurely
MODEL_SAVE_PATH = os.path.join(BACKEND_DIR, "models", "plant_disease_resnet_new.pt")
...
def train_resnet():
    ...
    log(f"Training ResNet18 model (ALL LAYERS UNFROZEN) on device: {device}...")
    ...
    log(f"Number of classes detected: {len(disease_classes)}")
    ...
    # Save model weights to new model path
    os.makedirs(os.path.dirname(MODEL_SAVE_PATH), exist_ok=True)
    torch.save(model.state_dict(), MODEL_SAVE_PATH)
    log(f"Saved unfrozen ResNet18 model weights to {MODEL_SAVE_PATH}")
